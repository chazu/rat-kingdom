...
//! `rk` binary can say which build it *is* and not merely which semver it
//! claims.
//!
//! Lives in `rk-cli` — the crate that produces `rk` (which also becomes the
//! daemon via `rk daemon run` and the MCP bridge via `rk mcp`, all in the
//! same process) — rather than in `rk-core`. `rk-core` is a dependency of
//! every crate in the workspace, so a build script there that changes its
//! `cargo:rustc-env` output on every commit invalidates rk-core's compiled
//! artifact on every commit, which in turn forces a relink of everything
//! that depends on it — the whole workspace — even for crates with no
//! interest in build identity. Scoping the stamp to each leaf executable's
//! own build script means only that executable's crate is rebuilt for a
//! commit-only change; unaffected shared libraries stay cached.
//! `rk_core::version::init_build_sha` carries the resulting value from
//! `main` into the runtime consumers that used to read a
//! `rk-core`-embedded constant.
//!
//! `rk-mcp` is the workspace's other shipped executable and needs the exact
//! same treatment; see `crates/rk-mcp/build.rs`, which duplicates this file
//! rather than sharing it through a crate — sharing it back would reproduce
//! the same fanout this split exists to avoid.
//!
...
/// rk-cli — and so relink the `rk` binary — for no change in output.
///
...
fn git(args: &[&str]) -> Option<String> {
    // Cargo sets `CARGO_MANIFEST_DIR` fresh in the environment on *every*
    // invocation of this build script, so reading it at runtime (rather than
    // baking it in at compile time via `env!`) keeps this correct after the
    // crate directory moves — a linked worktree relocated or promoted from a
    // shared cache, for instance. A cached build-script binary compiled with
    // `env!("CARGO_MANIFEST_DIR")` would keep running `git` against wherever
    // it was first built, silently falling back to "unknown" once that path
    // stopped existing even though the current checkout is perfectly valid.
