...
use rk_core::sdlc::{CiSignal, ConfiguredSourceName, Correlation, SignalEnvelope, SignalKind, SignalPayload};
...
async fn event(layout: &Layout, args: IngestEventArgs, json_out: bool) -> Result<()> {
    let envelope = if let Some(path) = &args.file {
        if args.kind.is_some() || args.delivery_id.is_some() || args.summary.is_some() {
            return Err(anyhow!("--file accepts a canonical envelope only; do not combine it with event fields"));
        }
    ...
    let result = client
        .call("ingest.event", json!({"source": args.source, "envelope": envelope}))
        .await?;
    if json_out {
        println!("{result}");
    } else {
        println!("accepted {}", result["receipt"]["receipt_id"].as_str().unwrap_or("?"));
    }
    Ok(())
}
...
async fn state(layout: &Layout, args: IngestStateArgs, json_out: bool) -> Result<()> {
    ...
    if json_out {
        ...
        println!("{} facts", result["facts"].as_array().map(|v| v.len()).unwrap_or(0));
    }
    Ok(())
}

fn build_ci_envelope(args: &IngestEventArgs) -> Result<SignalEnvelope> {
    let kind = match args.kind.as_deref().ok_or_else(|| anyhow!("--kind is required"))? {
    ...
    for pair in &args.attrs {
        let (key, value) = pair.split_once('=').ok_or_else(|| anyhow!("--attr must be key=value"))?;
    ...
    let envelope = SignalEnvelope {
        kind,
        source,
        delivery_id: args.delivery_id.clone().ok_or_else(|| anyhow!("--delivery-id is required"))?,
        occurred_at: now,
        observed_at: now,
...
fn reject_cli_attr_key(key: &str) -> Result<()> {
    ...
    let banned = ["secret", "token", "credential", "password", "authorization", "raw", "telemetry", "command", "action", "executable"];
    if banned.iter().any(|word| lower.contains(word)) {
        return Err(anyhow!("secret-like or executable attribute key rejected: {key}"));
    }
    Ok(())
}
