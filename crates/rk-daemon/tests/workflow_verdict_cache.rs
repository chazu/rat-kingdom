...
async fn run_workflow(client: &mut Client, repo_dir: &Path, head_sha: &str) -> String {
    let started = client
        .call(
            "workflow.run",
            json!({
...
/// `head_sha` the way steward.cue's reviewer prompt does (TKT-01M036N1RT74H6NPRH5FMM8A6T).
...
async fn same_sha_retry_consumes_cached_approve_without_spawning_a_reviewer() {
    ...
    plant_verdict(&mut client, &repo_name, "earlier-reviewer", "APPROVE", "sha-abc123").await;

    let id = run_workflow(&mut client, repo_dir.path(), "sha-abc123").await;
...
async fn a_different_commit_is_a_cache_miss_and_spawns_a_fresh_reviewer() {
    ...
    // for the new tip.
    plant_verdict(&mut client, &repo_name, "earlier-reviewer", "APPROVE", "sha-OLD").await;

    let id = run_workflow(&mut client, repo_dir.path(), "sha-NEW").await;
    ...
    plant_verdict(&mut client, &repo_name, &reviewer, "APPROVE", "sha-NEW").await;
...
async fn cached_rework_routes_to_rework_without_spawning_a_reviewer() {
    ...
    plant_verdict(&mut client, &repo_name, "earlier-reviewer", "REWORK", "sha-abc123").await;
