...
// spawn → wait → run (red, but piped through a successful consumer) → dismiss
// (never reached). `exit 3 | cat` is exactly the masking shape a `mise run
// verify 2>&1 | tail -100` produces: without `set -o pipefail` in effect, an
// `sh -c`/`bash -c` wrapper reports the LAST pipeline stage's exit status —
// here `cat`'s 0 — hiding the real failure behind a successful filter.
const RUN_RED_PIPED_THROUGH_SUCCESS_WORKFLOW: &str = r#"
workflow: {
    name: "run-red-piped"
    params: {taskId: {type: "string", required: true}}
    agents: {default: {harness: "fake", model: "sonnet"}}
    steps: [
        {type: "spawn", role: "rat", task: {title: _input.taskId, description: "do " + _input.taskId}},
        {type: "wait", timeout: "30s"},
        {type: "run", command: "exit 3 | cat", expectExit: 0, timeout: "30s"},
        {type: "dismiss"},
    ]
}
"#;

/// TKT-01M0H5JNZQKZ35V87Q4H4N3EPH: a failing check command piped through a
/// successful output consumer (`cat`, standing in for `tail`/`tee`/a
/// renderer) must still fail the gate closed on the check's OWN exit code,
/// not the consumer's. Proves RK's own `run` step wraps declared commands
/// with `set -o pipefail` so this masking shape cannot present a red check
/// as green — the same guarantee the completion protocol now demands of a
/// rat's own interactive verification.
#[tokio::test]
async fn run_step_fails_closed_on_a_masked_pipe_not_the_consumers_exit() {
    let _env_guard = HARNESS_ENV_LOCK.lock().await;
    let home = tempfile::tempdir().unwrap();
    let repo_dir = tempfile::tempdir().unwrap();
    git(repo_dir.path(), &["init", "-b", "main"]);
    ...
    std::fs::create_dir_all(&wf_dir).unwrap();
    std::fs::write(
        wf_dir.join("run-red-piped.cue"),
        RUN_RED_PIPED_THROUGH_SUCCESS_WORKFLOW,
    )
    .unwrap();
        ...
        match status["instance"]["status"].as_str().unwrap_or("") {
            "failed" => {
                ...
                assert!(
                    ...
                    "expected the pipe's real (masked) exit code 3, not cat's 0: {err}"
                );
                failed = true;
                break;
            }
            "completed" => panic!("a masked-pipe red run gate must not complete"),
            _ => {}
        }
    }
    assert!(
        failed,
        "run gate did not fail closed on the masked pipe's real exit code"
    );
    ...
    assert!(
        !listing.contains("work-"),
        "red work behind a masked pipe must not merge: {listing}"
    );
