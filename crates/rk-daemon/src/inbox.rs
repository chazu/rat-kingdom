...
//! (`open-suggestion`). It is here because a proposal that nobody votes on is
...
/// - **decayed** — `expires_at` has passed. Since TKT-168 most ballots are
///   durable and carry no `expires_at` at all, so this filter now only bites on
///   a ballot deliberately time-boxed with `--ttl` and on legacy Ephemeral ones
///   written before that change. It stays because expiry is collected by the GC
///   rather than filtered on read, so a scan still returns ballots whose window
///   closed minutes ago; endorsing one is not what the operator meant.
///
