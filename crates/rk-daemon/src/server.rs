...
#[cfg(test)]
mod cleared_branches_tests {
    //! Commit-count awareness at the "unreviewed landing" inbox-clearing call
    //! site (TKT-01M0CTC4DPFV7Q2642AZH354BV): `cleared_branches_for_paths`
    //! feeds `rk inbox`'s awaiting-review row clearing straight off
    //! `Repo::branch_merged_or_gone`. An empty branch — pushed/opened for
    //! review but never committed to — must not silently clear its row as if
    //! a human had reviewed and merged it.
    use super::*;
    use std::process::Command;
    ...
    #[test]
    fn an_empty_branch_does_not_clear_its_awaiting_review_row() {
        let repo_dir = tempfile::tempdir().unwrap();
        let p = repo_dir.path();
        init_repo(p);
        // Opened for review, never touched: tip == fork point.
        git(p, &["checkout", "-b", "nowork", "main"]);
        git(p, &["checkout", "main"]);

        let mut paths = HashMap::new();
        paths.insert("repo".to_string(), p.to_path_buf());
        let events = vec![(
            "repo".to_string(),
            "nowork".to_string(),
            "main".to_string(),
        ...
        assert!(
            cleared.is_empty(),
            "an empty, never-diverged branch must not read as reviewed/merged"
        );
    }

    #[test]
    fn a_genuinely_merged_branch_clears_its_awaiting_review_row() {
        let repo_dir = tempfile::tempdir().unwrap();
        let p = repo_dir.path();
        init_repo(p);
        ...
        let mut paths = HashMap::new();
        paths.insert("repo".to_string(), p.to_path_buf());
        let events = vec![("repo".to_string(), "work".to_string(), "main".to_string())];

        let cleared = cleared_branches_for_paths(events, paths);
        assert!(
            cleared.contains(&("repo".to_string(), "work".to_string())),
            "a genuinely merged branch must clear its row"
        );
    }
}
