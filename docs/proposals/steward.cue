// PROPOSAL (rat-29 workflow-review, landed): synchronized steward.cue.
// The land-result gate is applied in the shipped definition. Steward uses the
// repository delivery contract's `delivered: true` result so direct merges,
// merge+push, branch pushes, and PR hand-offs are accepted only when delivery
// succeeds; conflicts and push failures fail closed and remain visible.
// This proposal copy is synchronized with the shipped workflow, including its
// named checks, diff-scope guard, timeout routing, and reviewer binding.
//
