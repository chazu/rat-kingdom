// PROPOSAL (rat-29 workflow-review): revised steward.cue.
// Change vs shipped: the APPROVE case now gates the `land` RESULT. A `land`
// merge conflict or a moved target is a clean {merged: false} in
// ctx.previousResult — NOT an error (see #LandStep in schema.cue) — so the
// shipped steward completes the instance cleanly on a FAILED auto-merge,
// silently leaving the branch unmerged with no operator signal. The added
// `evaluate {expect: {merged: true}}` fails the instance closed on merged:false,
// surfacing the stuck auto-merge in `rk inbox` exactly like the two upstream
// gates already do. Schema doc for #LandStep explicitly prescribes this gate.
//
