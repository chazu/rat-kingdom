// PROPOSAL (rat-29 workflow-review): revised nightly-self-improve.cue.
// Two changes vs shipped:
//  1. ADD a per-instance `budget` cap (#WorkflowBudget). This is the unattended,
//     scheduled workflow most able to run up spend (groom + a parallel drain +
//     refine, all overnight with no operator watching). The shipped comment says
//     cost is "bounded by the fleet/repo budget caps" — but those are FLEET-wide,
//     not scoped to this run; a single runaway night can consume the whole
//     fleet's cap. A per-instance `budget` bounds THIS night specifically.
//  2. DROP the dead `repo` param — the phases derive their repo from the run
//     context and $RK_REPO, never from this param (see backlog-drain proposal).
//
...
workflow: {
	...
	params: {
		...
		// dispatch is refused — below the global fleet/repo caps.
