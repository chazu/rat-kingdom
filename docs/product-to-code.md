...
step crosses the Phase 2 canonical approval boundary: the CLI only proposes, and
the daemon alone applies changes after an authenticated operator approval with
status, digest, and compare-and-swap (CAS) checks.
...
5. **Propose the daemon graph apply.** The CLI submits a canonical
   `ticket_graph.apply` proposal and waits for an authenticated operator
   approval. No local mutation occurs.
...
8. **Propose `product_to_code.dispatch`.** The CLI submits a canonical dispatch
   proposal referencing the approved graph-apply execution and waits for an
...
rk product-to-code research validate --artifact research.json

# Validate the ticket graph (rejects cycles and missing dependencies).
rk product-to-code graph validate --graph graph.json

# Dry-run the graph to preview the mint plan.
rk product-to-code graph dry-run --graph graph.json

# Propose the daemon graph apply (waits for operator approval; no local mutation).
rk product-to-code graph propose-apply --graph graph.json

# Propose implementation dispatch for unblocked minted tickets.
...
rk product-to-code verify-report validate --report report.json
...
- **Proposal approval is the Phase 2 canonical boundary.** Every mutating step
  ...
  status, digest, and CAS verification. The CLI is thin proposal wiring.
