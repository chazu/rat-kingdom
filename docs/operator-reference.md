...
Start with the [first-repository walkthrough](first-repository.md). This catalog
retains advanced commands, configuration, automation and integration details.
...
Structured harness events drive the lifecycle.
...
For source navigation and release evidence, see the
[current architecture and acceptance map](architecture.md).
...
The dashboard auto-starts the daemon and opens a live interactive Rust terminal UI with overview, agents, workflows, tickets, inbox, approvals, and events panels. Use `tab` or the arrow keys to switch panels, `j`/`k` to scroll, `r` to refresh, and `q` to quit. `--plain` emits one bounded Markdown snapshot for pipes, while `rk --json factory dashboard ...` emits the typed machine-readable envelope. Use `rk factory render --snapshot snapshot.json --events events.json --output dashboard.md` to render saved native evidence offline for reports or a Jcode side panel. Native typed automation uses `rk --json factory ...` or `rk-mcp`; exact digest approval is daemon-enforced for that factory path, but legacy `workflow.run` is not globally gated. Scorecards and recommendations are read-only and advisory: they use structured sources, treat missing families as unobserved, keep static routing unchanged, and cannot mutate policy, config, workflows, tickets, approvals, or dispatch. See [docs/factory-foreman.md](factory-foreman.md) for authority, CLI/MCP schemas, scorecard schema, recommendation thresholds, snapshot/replay/watch semantics, recovery, and limitations.
...
[docs/reactor.md](reactor.md#built-in-reaction-resolution-backlinks).
...
policy](repository-policy.md) for the schema and trust boundary.

For the operator walkthrough, see [Repository onboarding](repo-onboarding.md).
...
[docs/repository-policy.md](repository-policy.md) for configuration and
[docs/pr-merge-mode.md](pr-merge-mode.md) for forge credential details.
...
The operation is durable before Git advances. A failed registry, ticket or
evidence write reports an unsettled operation; retry the printed
`rk revert <agent> --operation <id>` command with the same `--block` choice.
Retries settle the original commit and completion fact, including across daemon
restart or agent-name reuse, and never reopen later work again.
