...
- Reactor: `crates/rk-daemon/src/reactor.rs` (`Reactor::run_cycle`, plus the
  built-ins `Reactor::promote_conventions`, `Reactor::coalesce_obstacles`, and the
  resolution backlinks `Reactor::link_resolution` / `Reactor::steer_from_resolution`),
...
- Tests: `crates/rk-daemon/tests/reactor.rs` (live-daemon fire, idempotency under
  ...
  re-filing — and resolution backlinks: retire-and-lay-trail plus
